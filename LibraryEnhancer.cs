...
internal static class LibraryEnhancer
{
    ...
    static void AttachWhenReady(object? sender, EventArgs e)
    {
        ...
        if (form == null) return;
        cards = FindControls<FlowLayoutPanel>(form).FirstOrDefault();
        if (cards == null) return;

        attached = true;
        Application.Idle -= AttachWhenReady;
        EnsureFavoritesLibrary();
        BuildSidebar(form);
    ...
    static void EnsureFavoritesLibrary()
    {
    ...
    static void ApplyLibraryFilter()
    {
        ...
        if (countLabel != null)
            countLabel.Text = $"{visible} jeu(x) visible(s)";
    }

    static bool IsInAnyLibrary(string gameName) => LibraryStore.Load()
        .Where(x => !x.Name.Equals("★ Favoris", StringComparison.OrdinalIgnoreCase))
        .Any(x => x.Games.Any(g => g.Equals(gameName, StringComparison.OrdinalIgnoreCase)));
    ...
    static void WireCards() 
    {
        ...
        foreach (Control card in cards.Controls)
        {
            if (card.Tag is not Game game || wiredCards.Contains(card)) continue;
            wiredCards.Add(card);
            AttachMenu(card, game);
        }
    }

    static void RebuildCardMenus()
    {
        if (cards == null) return;
        wiredCards.Clear();
        WireCards();
    ...
    static void AttachMenu(Control card, Game game)
    {
    ...
    static ContextMenuStrip BuildContextMenu(Game game)
    {
        ...
        foreach (var library in LibraryStore.Load().Where(x => !x.Name.Equals("★ Favoris", StringComparison.OrdinalIgnoreCase)))
        {
            ...
            item.Click += (_, _) =>
            {
                if (LibraryStore.Contains(library.Name, game.Name))
                    LibraryStore.RemoveGame(library.Name, game.Name);
                else
                    LibraryStore.AddGame(library.Name, game.Name);
                RefreshLibraries();
                ApplyLibraryFilter();
            };
        ...
        var createFromMenu = new ToolStripMenuItem("Créer une bibliothèque…");
        createFromMenu.Click += (_, _) => CreateLibraryDialog();
        add.DropDownItems.Add(new ToolStripSeparator());
        add.DropDownItems.Add(createFromMenu);
        menu.Items.Add(add);
    ...
    static void ManageLibrariesDialog()
    {
        ...
        delete.Click += (_, _) =>
        {
            ...
            if (MessageBox.Show($"Supprimer « {library.Name} » ? Les jeux ne seront pas désinstallés.", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                return;

            LibraryStore.Delete(library.Name);
            selectedLibrary = "Tous les jeux";
            dialog.Close();
            RefreshLibraries();
            ApplyLibraryFilter();
        };
