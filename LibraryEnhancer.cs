...
internal static class LibraryEnhancer
{
    ...
    static string selectedLibrary = "Toutes les jeux";
    ...
    static void Attach(Form target)
    {
        attached = true;
        cards = target.Controls.OfType<FlowLayoutPanel>().FirstOrDefault();
        if (cards == null) { attached = false; return; }
        ...
        refreshTimer.Tick += (_, _) => ApplyLibraryFilter();
        refreshTimer.Start();
    }
    ...
    static void RefreshLibraries()
    {
        ...
        var custom = LibraryStore.Load().Select(x => x.Name)
            .Where(x => !x.Equals("★ Favoris", StringComparison.OrdinalIgnoreCase))
            .OrderBy(x => x, StringComparer.OrdinalIgnoreCase)
            .ToList();

        var names = new List<string> { "Toutes les jeux", "★ Favoris", "Non classés" };
    ...
    static void ApplyLibraryFilter()
    {
        ...
        foreach (Control control in cards.Controls)
        {
            ...
            var show = selectedLibrary.Equals("Toutes les jeux", StringComparison.OrdinalIgnoreCase)
    ...
    static bool IsInAnyLibrary(string game)
    {
        return LibraryStore.Load().Any(l => l.Games.Any(g => g.Equals(game, StringComparison.OrdinalIgnoreCase)));
    }
    ...
    static void WireCards()
    {
        ...
        {
            if (card.Tag is not Game game || card.Tag is LibraryCardMarker) continue;
            card.ContextMenuStrip = BuildContextMenu(game);
            foreach (Control child in card.Controls)
                child.ContextMenuStrip = card.ContextMenuStrip;
            card.Tag = new LibraryCardMarker(game);
        }
    }
    ...
    static ContextMenuStrip BuildContextMenu(Game game)
    {
        ...
        var add = new ToolStripMenuItem("Ajouter à une bibliothèque");
        foreach (var library in LibraryStore.Load().Where(x => !x.Name.Equals("★ Favoris", StringComparison.OrdinalIgnoreCase)))
        {
            var item = new ToolStripMenuItem(library.Name) { Checked = LibraryStore.Contains(library.Name, game.Name) };
            item.Click += (_, _) =>
            {
                if (item.Checked) LibraryStore.RemoveGame(library.Name, game.Name);
        ...
        if (add.DropDownItems.Count == 0)
            add.DropDownItems.Add(new ToolStripMenuItem("Créer une bibliothèque…") { Enabled = true });
        menu.Items.Add(add);
    ...
    static void CreateLibraryDialog()
    {
        ...
        if (dialog.ShowDialog(form) == DialogResult.OK)
        {
            ...
            {
                if (!LibraryStore.Create(name)) MessageBox.Show("Cette bibliothèque existe déjà.", "Mataiasu Launcher", MessageBoxButtons.OK, MessageBoxIcon.Information);
                RefreshLibraries();
            }
        }
    }
    ...
    static void ManageLibrariesDialog()
    {
        ...
        var list = new ListBox { Dock = DockStyle.Top, Height = 300, BackColor = Color.FromArgb(31, 27, 45), ForeColor = Color.White, BorderStyle = BorderStyle.None };
        list.Items.AddRange(libs.Select(x => $"{x.Name}   ·   {x.Games.Count} jeu(x)").Cast<object>().ToArray());
        var delete = new Button { Text = "Supprimer la bibliothèque sélectionnée", Dock = DockStyle.Bottom, Height = 42, FlatStyle = FlatStyle.Flat, BackColor = Color.FromArgb(70, 48, 84), ForeColor = Color.White };
        ...
        dialog.Controls.Add(delete); dialog.Controls.Add(list);
        dialog.ShowDialog(form);
    }

    sealed class LibraryCardMarker
    {
        public Game Game { get; }
        public LibraryCardMarker(Game game) => Game = game;
    }
}
